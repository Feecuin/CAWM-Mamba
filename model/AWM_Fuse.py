...
class VSSBlock(nn.Module):
    def __init__(
        ...
        self.conv_blk = CAB(hidden_dim)
        self.ln_2 = nn.LayerNorm(hidden_dim)
        self.upsamle = SimpleSRModule(in_channels=hidden_dim, out_channels=hidden_dim, upscale_factor=2)
        # self.GELU = nn.GELU()
...
class WaveMamba(nn.Module):
    ...
    def forward(self, inp_img, inp_rain):
        _, _, H, W = inp_img.shape
 
        inp_enc_level1_one = self.patch_embed(inp_rain)  # b, hw, c
        inp_enc_level1_two = self.patch_embed(inp_img)  # b, hw, c
        ...
        ##########################
            ...
            out_dec_level1_one = self.output(out_dec_level1_one)  + inp_img + inp_rain
            out_dec_level1_two = self.output(out_dec_level1_two) + inp_img + inp_rain 
