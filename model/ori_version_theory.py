...
class Interaction_block(nn.Module):
    def __init__(self,num_feat):
        ...
        # self.conv_degradation = nn.Conv2d(in_channels=num_feat, out_channels=num_feat, kernel_size=1, stride=1, padding=0) 
    ...
    def generate_attention(self, deg_feat):
        """生成动态权重"""
        return torch.sigmoid(self.deg_proj(deg_feat))        
    def forward(self,vi_rain,ir,degradation_features):
        # print(ir.shape)
        # print("x",vi_rain.shape)
        ir = self.conv1(ir) #  ir's dim the same as vi_rain
        # print(ir.shape)
        B, _, H, W = ir.shape
        vi_rain = self.conv1_1(vi_rain) # vi_rain's dim is 3
        C = ir.shape[1]
        # deg_map = degradation_features.unsqueeze(-1).unsqueeze(-1).expand(-1, C, H, W)
        # ir = ir * deg_map
        deg_attn = self.generate_attention(degradation_features)  # [B, C]
        ir = ir * (1 + 0.1 * deg_attn.view(B, -1, 1, 1))
        vi_rain = vi_rain * (1 - 0.1 * deg_attn.view(B, -1, 1, 1))
        ir_mp = ir[:, :C//2, :, :]
        # print("ir_mp",ir_mp.shape)
        ir_gmp = ir[:, C//2:, :, :]
        # print("ir_gmp",ir_gmp.shape)
        vi_rain_ap = vi_rain[:, :C//2, :, :]
        vi_rain_gap = vi_rain[:, C//2:, :, :]
        # print(vi_rain_gap.shape)
        ...
        # print("vi_rain_app",vi_rain_app.shape)
        # print("vi_rain_gap_conv",vi_rain_gap_conv.shape)
        ir_gather  = ir_mp_app * ir_gmp_conv
        vi_gather  = vi_rain_app * vi_rain_gap_conv
        
        # ir_gather = self.upsamle2(ir_gather)
        # vi_gather = self.upsamle1(vi_gather)
...
class WaveletDecomposition(nn.Module):
    ...
    def forward(self, img):
        ...
        LH = yh[0][:, :, 0, :, :].permute(0, 2, 3, 1).contiguous()
        HL = yh[0][:, :, 1, :, :].permute(0, 2, 3, 1).contiguous()
        HH = yh[0][:, :, 2, :, :].permute(0, 2, 3, 1).contiguous()
...
#         寻找共同退化空间的模块
#         Args:
#             input_dim (int): 输入通道数（如退化图像的通道数）。
#             hidden_dim (int): 隐藏层维度（输出特征向量的维度）。
#         """
#         super(DegradationEncoder, self).__init__()
#         self.encoder = nn.Sequential(
#             nn.Conv2d(input_dim, hidden_dim, kernel_size=3, stride=1, padding=1),  # 提取局部特征
#             nn.ReLU(inplace=True),
#             nn.Conv2d(hidden_dim, hidden_dim, kernel_size=3, stride=1, padding=1),  # 深化特征
#             nn.ReLU(inplace=True)
#         )
#         self.global_pool = nn.AdaptiveAvgPool2d(1)  # 全局平均池化
#         self.fc = nn.Linear(hidden_dim, hidden_dim)  # 压缩到固定维度

#     def forward(self, x):
#         """
#         Args:
#             x (torch.Tensor): 输入退化图像，形状为 (B, C, H, W)。
#         Returns:
#             torch.Tensor: 退化的共同特征向量，形状为 (B, hidden_dim)。
#         """
#         features = self.encoder(x)  # 提取局部特征
#         pooled_features = self.global_pool(features).view(features.size(0), -1)  # 全局特征聚合
#         latent = self.fc(pooled_features)  # 压缩到固定维度
#         return latent

class DegModulator(nn.Module):
    def __init__(self, dim):
        super().__init__()
        self.modulator = nn.Sequential(
            nn.Linear(dim, dim*2),
            nn.ReLU(inplace=True),
            nn.Linear(dim*2, 4),  # 4个频带的调制系数
            nn.Sigmoid()
        )
    
    def forward(self, x):
        coeffs = self.modulator(x)
        return {
            'll': coeffs[:, 0:1],  # 低频，控制雾的恢复
            'lh': coeffs[:, 1:2],  # 水平高频
            'hl': coeffs[:, 2:3],  # 垂直高频
            'hh': coeffs[:, 3:4]   # 对角高频，控制雨雪的恢复
        }


class EnhancedDegradationEncoder(nn.Module):
    def __init__(self, input_dim, hidden_dim):
        super(EnhancedDegradationEncoder, self).__init__()
        ...
        # IR引导
        self.ir_guidance = nn.Conv2d(input_dim, hidden_dim, 1)
    ...
    def forward(self, x_vis, x_ir):
        # 局部特征
        local_feat = self.local_encoder(x_vis)
        
        # 全局特征
        gap_feat = self.gap(local_feat).squeeze(-1).squeeze(-1)
        gmp_feat = self.gmp(local_feat).squeeze(-1).squeeze(-1)
        
        # IR引导特征
        ir_feat = self.gap(self.ir_guidance(x_ir)).squeeze(-1).squeeze(-1)
        
        # 特征融合
        feat = torch.cat([gap_feat, gmp_feat, ir_feat], dim=1)
        deg_feat = self.fusion(feat)
...
class WaveMamba(nn.Module):
    def __init__(self,
        ...
        # self.degradation_encoder = DegradationEncoder(input_dim=3, hidden_dim=dim)
        self.degradation_encoder = EnhancedDegradationEncoder(input_dim=3, hidden_dim=dim)
        # self.deg_modulator = DegModulator(dim)
    def forward(self, inp_rain,inp_img):
        _, _, H, W = inp_img.shape
        # print("inp_img shape:",inp_img.shape)
        # print(inp_img.device)
        degradation_features = self.degradation_encoder(inp_rain,inp_img)
        # degradation_features = degradation_features.unsqueeze(-1).unsqueeze(-1).expand(-1, -1, H, W)
        # inp_img = torch.cat([inp_img, degradation_features], dim=1)
        # print("inp_img shape:",inp_img.shape)
        # print("inp_rain shape:",inp_rain.shape)
        inp_enc_level1 = self.interactions(inp_rain,inp_img,degradation_features)
